...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.embeds = nn.Embedding(vocab_size, model_dim)
        ...
        self.Ww = nn.Linear(model_dim, vocab_size)
    ...
    def _get_positional_encodings(self, seq_len, d_model):
        ...
        return torch.round(PE, decimals=5)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        h = self.embeds(context) + self.position_embeddings(positions)

        # 2. Pass through transformer blocks
        h = self.blocks(h)

        # 3. Apply final LayerNorm, then project to vocab_size
        h = self.Ww(self.norm(h))
